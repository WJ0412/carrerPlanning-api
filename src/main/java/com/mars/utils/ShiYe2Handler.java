...
public class ShiYe2Handler {
    private Configuration configuration = null;

    public ShiYe2Handler() {
    ...
    public String createShiYe(HttpServletRequest request, Map<String, String> maps) {
        ...
        String fileName=maps.get("studentid")+"事业规划修订.doc";
        File outFile = new File(folder, fileName);  //输出文档路径及名称
        System.out.println(outFile.getAbsoluteFile().getAbsolutePath());
        Writer out = null;
        try {
        ...
        try {
            t = configuration.getTemplate("shiye2.ftl");
            t.setEncoding("utf-8");
