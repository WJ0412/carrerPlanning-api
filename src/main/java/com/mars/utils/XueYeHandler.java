...
public class XueYeHandler {
    ...
    public String createXueYe(HttpServletRequest request, Map<String, String> maps) {
        ...
        if (maps.isEmpty()){
            System.out.println("该未填写学业规划（一）");
            return null;
        }
        if (maps.get("target")!=null){
            dataMap.put("target",maps.get("target"));
        }else{
            dataMap.put("target","未填写");
        }
        ...
        String fileName=maps.get("studentid")+"学业规划（一）.doc";
        ...
        try {
            t = configuration.getTemplate("xueye1.ftl");
            t.setEncoding("utf-8");
